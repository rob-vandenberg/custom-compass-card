import{LitElement,html,svg,css}from"https://unpkg.com/lit@2.0.0/index.js?module";const CARD_VERSION="3.2.101",DEFAULT_CONFIG={compass_entity:"sun.sun",compass_attribute:"azimuth",compass_adjustment:0,background_color:"#101010",bezel_color:"#383838",bezel_width:16,bezel_size:0,needle_show:!0,needle_invert:!1,needle_rotate:!1,needle_color_1:"#FF0000",needle_color_1_pos:50,needle_color_2:"#EEEEEE",needle_color_2_pos:50,needle_height:100,needle_width:10,needle_position:-10,needle_morph:50,needle_curve:0,cardinals_show:!1,cardinal_north:"N",cardinal_east:"E",cardinal_south:"S",cardinal_west:"W",cardinals_fontsize:10,cardinals_fontweight:400,cardinals_position:1.5,cardinals_fontcolor:"#EEEEEE",major_ticks_show:!0,major_ticks_length:6,major_ticks_width:2,major_ticks_position:-3.5,major_ticks_color:"#CCCCCC",minor_ticks_show:!0,minor_ticks_length:3,minor_ticks_width:1.5,minor_ticks_position:-4.5,minor_ticks_color:"#AAAAAA",micro_ticks_show:!0,micro_ticks_length:0,micro_ticks_width:2,micro_ticks_position:-6.5,micro_ticks_color:"#888888",header_show:!1,header_text:"header",header_fontsize:2,header_fontweight:400,header_position:0,header_fontcolor:"#FFFFFF",footer_show:!1,footer_text:"footer",footer_fontsize:2,footer_fontweight:400,footer_position:0,footer_fontcolor:"#FFFFFF",field_1_show:!0,field_1_template:"${compass_direction}",field_1_fontsize:1.5,field_1_fontweight:400,field_1_position:23,field_1_fontcolor:"#29B6CF",field_1_unit:"",field_1_unit_fontsize:1,field_1_unit_fontweight:400,field_1_unit_fontcolor:"#196D7C",field_2_show:!1,field_2_template:"{{ states('sensor.ws_wind_speed') | round(1) }}",field_2_unit:"km/h",field_2_fontsize:2,field_2_fontweight:400,field_2_position:50,field_2_fontcolor:"#E8E8E8",field_2_unit_fontsize:1.2,field_2_unit_fontweight:400,field_2_unit_fontcolor:"#8C8C8C",field_3_show:!0,field_3_template:"{{ state_attr('sun.sun', 'azimuth') | round(0) }}",field_3_unit:"°",field_3_fontsize:1.4,field_3_fontweight:400,field_3_position:79,field_3_fontcolor:"#808080",field_3_unit_fontsize:1.4,field_3_unit_fontweight:400,field_3_unit_fontcolor:"#606060"};class CustomCompassCardEditor extends LitElement{static properties={hass:{type:Object},_config:{type:Object}};setConfig(e){this._config={...DEFAULT_CONFIG,...e},this.loadCardHelpers()}async loadCardHelpers(){if(!window.customElements.get("ha-entity-picker")){const e=await window.loadCardHelpers(),t=await e.createCardElement({type:"entities",entities:[]});await t.constructor.getConfigElement()}}_valueChanged(e,t){if(!this._config||!this.hass)return;let i;i=void 0!==t.detail?.value?t.detail.value:"HA-SWITCH"===t.target.tagName?t.target.checked:t.target.value,"number"===t.target.type&&(i=parseFloat(i),isNaN(i))||(this._config={...this._config,[e]:i},this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0})))}_colorPicker(e,t){const i=this._config[e]||"#ffffff";return html`
  ...
  `}customElements.define("custom-compass-card-editor",CustomCompassCardEditor);class CustomCompassCard extends LitElement{static properties={hass:{type:Object},config:{type:Object},_degrees:{type:Number},_field1Value:{type:String},_field2Value:{type:String},_field3Value:{type:String},_headerValue:{type:String},_footerValue:{type:String},_error:{type:Boolean}};constructor(){super(),this._degrees=0,this._prevDegrees=null,this._templatesDirty=!1,this._field1Value="",this._field2Value="",this._field3Value="",this._headerValue="",this._footerValue="",this._error=!1}setConfig(e){if(!e.compass_entity)throw new Error("You must define a compass_entity.");this.config={...DEFAULT_CONFIG,...e}}willUpdate(e){if(!e.has("hass")||!this.config?.compass_entity)return;const t=this.hass.states[this.config.compass_entity];if(t){let e=t.state;const i=this.config.compass_attribute;if(i&&"string"==typeof i&&""!==i.trim()){const l=t.attributes[i.trim()];null===l||isNaN(parseFloat(l))||(e=l)}const l=parseFloat(e);if(isNaN(l))this._degrees=0,this._prevDegrees=null,this._templatesDirty=!0,this._error=!0;else{const e=((l+(parseFloat(this.config.compass_adjustment)||0))%360+360)%360;e!==this._prevDegrees&&(this._degrees=e,this._prevDegrees=e,this._templatesDirty=!0),this._error=!1}}else this._degrees=0,this._prevDegrees=null,this._templatesDirty=!0,this._error=!0}async _updateTemplates(){this._templatesDirty=!1;const e=[{index:1,show:this.config.field_1_show,template:this.config.field_1_template},{index:2,show:this.config.field_2_show,template:this.config.field_2_template},{index:3,show:this.config.field_3_show,template:this.config.field_3_template}];for(const t of e)t.show?this._error?this[`_field${t.index}Value`]="Error":t.template&&(this[`_field${t.index}Value`]=await this._evaluateTemplate(t.template,this._degrees)):this[`_field${t.index}Value`]="";this.config.header_show&&this.config.header_text?this._headerValue=await this._evaluateTemplate(this.config.header_text,this._degrees):this._headerValue="",this.config.footer_show&&this.config.footer_text?this._footerValue=await this._evaluateTemplate(this.config.footer_text,this._degrees):this._footerValue=""}updated(e){super.updated(e),this._scaleElements(),this._templatesDirty&&this._updateTemplates()}_scaleElements(){const e=this.shadowRoot.querySelector(".compass-circle");if(!e)return;const t=e.offsetWidth,i=t/120;this.style.setProperty("--cc-font-size",.08*t+"px");const l=parseFloat(this.config.bezel_width),a=parseFloat(this.config.bezel_size);this.style.setProperty("--cc-circle-border-width",l*i+"px"),this.style.setProperty("--cc-circle-color",this.config.bezel_color),this.style.setProperty("--cc-bg-color",this.config.background_color),this.style.setProperty("--cc-border-size",`${a}px`);const o=parseFloat(this.config.needle_width),s=parseFloat(this.config.needle_height),n=parseFloat(this.config.needle_position);this.style.setProperty("--cc-needle-width",o*i+"px"),this.style.setProperty("--cc-needle-height",s*i+"px"),this.style.setProperty("--cc-needle-position",n*i+"px");const d=this.shadowRoot.querySelector(".compass-ticks-wrapper");d&&this.style.setProperty("--cc-circle-size",`${d.offsetWidth}px`)}async _evaluateTemplate(e,t){try{const i=e.replace("${compass_direction}",this.getCompassDirection(t));if(!i.includes("{{"))return i;return await this.hass.callApi("POST","template",{template:i})}catch(t){return console.error("CustomCompassCard: Error evaluating template:",e,t),"Error"}}getCompassDirection(e){const t=this.config.cardinal_north,i=this.config.cardinal_east,l=this.config.cardinal_south,a=this.config.cardinal_west;return[t,t+t+i,t+i,i+t+i,i,i+l+i,l+i,l+l+i,l,l+l+a,l+a,a+l+a,a,a+t+a,t+a,t+t+a][Math.floor((e+11.25)/22.5)%16]}_buildNeedlePath(e,t,i,l){let a=[{x:50,y:0},{x:0,y:50},{x:50,y:50+e},{x:100,y:50}];const o=Math.abs(e)/50,s=Math.sign(e)*Math.min(1,Math.abs(e)/50)*(Math.PI/2);let n=[{in:{x:1,y:0},out:{x:-1,y:0},lengthMultiplier:1},{in:{x:0,y:-1},out:{x:Math.cos(s),y:Math.sin(s)},lengthMultiplier:1},{in:{x:-1,y:0},out:{x:1,y:0},lengthMultiplier:o},{in:{x:-Math.cos(s),y:Math.sin(s)},out:{x:0,y:-1},lengthMultiplier:1}];i&&(a.forEach(e=>{e.y=-e.y}),n.forEach(e=>{e.in.y=-e.in.y,e.out.y=-e.out.y}));const d=a.map((e,i)=>{const l=t*n[i].lengthMultiplier;return{in:{x:e.x+n[i].in.x*l,y:e.y+n[i].in.y*l},out:{x:e.x+n[i].out.x*l,y:e.y+n[i].out.y*l}}}),r=[...a];d.forEach(e=>r.push(e.in,e.out));const c=-Math.min(...r.map(e=>e.y))+l;a.forEach(e=>{e.y+=c}),d.forEach(e=>{e.in.y+=c,e.out.y+=c});let h=`M ${a[0].x},${a[0].y}`;for(let e=0;e<a.length;e++){const t=(e+1)%a.length;h+=` C ${d[e].out.x},${d[e].out.y} ${d[t].in.x},${d[t].in.y} ${a[t].x},${a[t].y}`}h+=" Z";const _=[...a];d.forEach(e=>_.push(e.in,e.out));const f=_.map(e=>e.x),p=_.map(e=>e.y);return{path:h.trim().replace(/\s+/g," "),bounds:{minX:Math.min(...f),maxX:Math.max(...f),minY:Math.min(...p),maxY:Math.max(...p)}}}_renderTicks(){const e=this.config,t=50,i=[e.cardinal_north,e.cardinal_east,e.cardinal_south,e.cardinal_west],l=[0,4,8,12],a={major:{show:e.major_ticks_show,length:e.major_ticks_length,width:e.major_ticks_width,color:e.major_ticks_color,position:e.major_ticks_position||0,cardinals:e.cardinals_show,cardinal_color:e.cardinals_fontcolor,cardinal_fontsize:e.cardinals_fontsize,cardinal_fontweight:e.cardinals_fontweight,cardinal_position:e.cardinals_position||0},minor:{show:e.minor_ticks_show,length:e.minor_ticks_length,width:e.minor_ticks_width,color:e.minor_ticks_color,position:e.minor_ticks_position||0,cardinals:!1},micro:{show:e.micro_ticks_show,length:e.micro_ticks_length,width:e.micro_ticks_width,color:e.micro_ticks_color,position:e.micro_ticks_position||0,cardinals:!1}},o=Array.from({length:16},(e,o)=>{const s=a[o%4==0?"major":o%2==0?"minor":"micro"],n=s.show,d=parseFloat(s.length),r=parseFloat(s.width),c=s.color,h=parseFloat(s.position),_=22.5*o*Math.PI/180,f=Math.sin(_),p=Math.cos(_);if(s.cardinals){const e=l.indexOf(o),a=i[e],n=50+(t+s.cardinal_position)*f,d=50-(t+s.cardinal_position)*p,r=.85*s.cardinal_fontsize;return{show:!0,type:"text",x:n-r*f,y:d+r*p,letter:a,fontSize:s.cardinal_fontsize,fontWeight:s.cardinal_fontweight,color:s.cardinal_color}}return{show:n,type:"line",x1:50+(t+h)*f,y1:50-(t+h)*p,x2:50+(t+h-d)*f,y2:50-(t+h-d)*p,color:c,width:r}});return html`
      ...
      <ha-card>
        ...
        <div class="compass-container">
          <div class="compass-circle">
            ${l(i[0],this._field1Value)}
            ${l(i[1],this._field2Value)}
            ${l(i[2],this._field3Value)}
          </div>
          ${this._renderTicks()}
              ...
              <svg class="compass-needle"
                ...
                <defs>
                  <linearGradient id="needleGradient" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="${p}%" stop-color="${f}" />
                    <stop offset="${u}%" stop-color="${g}" />
